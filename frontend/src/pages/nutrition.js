...
const Nutrition = () => {
    ...
    const sendData = (file, imgDataUrl) => {
        ...
        axios.post('http://127.0.0.1:8000/api/nutrition/', formData, {
            headers: {
    ...
    const getClassificationResult = (obj, imgDataUrl) => {
        axios.get(`http://127.0.0.1:8000/api/nutrition/${obj.data.id}/`, {
            headers: {
                'accept': 'application/json',
            }
        })
    ...
    return (
        <React.Fragment>
            <Head>
                <title>
                    Nutrition Page | Ovarian Cancer Risk Prediction
                </title>
            </Head>
            <Box 
